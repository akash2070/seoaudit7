...
async function analyzeSpeed(url) {
  ...
  try {
    const apiUrl = `https://www.googleapis.com/pagespeedinspights/v5/runPagespeed?url=${encodeURIComponent(url)}&key=${apiKey}&strategy=mobile`;
...
async function analyzeLinks(url) {
  try {
    ...
    return {
      ...
      links: links.slice(0, 20) // Return first 20 for display
    };
  } catch (error) {
